...
class Model:
    def __init__(self, mesh: Mesh, constraints: Union[Constraint, List[Constraint]] = None,
                 ...
                 lumped_masses: Union[None, List[Union[int, float]]] = None, options: dict = None):
        ...
        assert lumped_masses is None or isinstance(lumped_masses, list)

        self.lumped_masses = lumped_masses if lumped_masses is not None else np.zeros(self.mesh.n_dof)
    ...
    def update_model(self):
        ...
        self.dof_masses = np.zeros(self.n_dof) if self.lumped_masses is None else self.lumped_masses
    ...
    def deformations(self):
        return np.array(
            [self.displacements(i_dof + 1) - self.displacements(i_dof) for i_dof in range(self.n_dof - 1)]).T

    def velocities(self, i_dof: int):
        return self.sol.y[self.n_dof + i_dof]

    def accelerations(self, i_dof: int, prepend: Union[ndarray, Iterable, int, float, None] = np._NoValue,
                      append: Union[ndarray, Iterable, int, float, None] = np._NoValue):
        return np.diff(self.velocities(i_dof), prepend=prepend, append=append) \
               / np.diff(self.sol.t, prepend=prepend, append=append)
    ...
    def linearize(self):
        m_matrix = np.diag(self.lumped_masses)
        k_matrix = np.zeros((self.n_dof, self.n_dof))
        c_matrix = np.zeros((self.n_dof, self.n_dof))
        for element in self.mesh.elements:
            if element.element_type == 'k':
                i, j, value = element.i, element.j, element.props['value']
                k_matrix[i, i] += value
                k_matrix[j, j] += value
                k_matrix[i, j] += -value
                k_matrix[j, i] += -value
        ...
        m_matrix = np.delete(m_matrix, self.constraining, axis=0)  # Remove rows
        m_matrix = np.delete(m_matrix, self.constraining, axis=1)  # Remove columns
        k_matrix = np.delete(k_matrix, self.constraining, axis=0)  # Remove rows
        k_matrix = np.delete(k_matrix, self.constraining, axis=1)  # Remove columns
        c_matrix = np.delete(c_matrix, self.constraining, axis=0)  # Remove rows
        c_matrix = np.delete(c_matrix, self.constraining, axis=1)  # Remove columns

        m_matrix_inv = np.linalg.inv(m_matrix)
        top_left = np.zeros((self.n_dof - len(self.constraining), self.n_dof - len(self.constraining)))
        top_right = np.eye(self.n_dof - len(self.constraining))
        bottom_left = -m_matrix_inv @ k_matrix
        bottom_right = -m_matrix_inv @ c_matrix
        ssa_matrix = np.block([[top_left, top_right],
                               [bottom_left, bottom_right]])
        n_freedof = self.n_dof - len(self.constraining)
        ssb_matrix = np.vstack((np.zeros((n_freedof, n_freedof)),
                                -np.linalg.inv(m_matrix)))
        ssc_matrix = np.eye(2 * n_freedof)
        ssd_matrix = np.zeros((2 * n_freedof, ssb_matrix.shape[1]))
        u = np.zeros((self.n_dof, self.loads[0].force.shape[0]))
        for load in self.loads:
            for load_dof in load.dof_s:
                u[load_dof, :] += load.force
        u = np.delete(u, self.constraining, axis=0)
        self.linear_model = {'k_matrix': k_matrix,
                             'c_matrix': c_matrix,
                             'm_matrix': m_matrix,
                             'lin_sys': (ssa_matrix, ssb_matrix, ssc_matrix, ssd_matrix),
                             'u': u}
        ...
        class Sol:
            def __init__(self, lin_solu_, model=self):
                ...
                for i_dof in range(model.n_dof):
                    if i_dof not in model.constraining:
                        self.y[i_dof, :] = d[i, :]
                        self.y[model.n_dof + i_dof, :] = v[i, :]
                        i += 1

        self.sol = Sol(lin_solu)
